...
public class Debug {
    ...
    private static void debugDetectedTables(Graphics2D g, PDDocument document, Page page) {
        NurminenDetectionAlgorithm detectionAlgorithm = new NurminenDetectionAlgorithm(document);
    ...
    public static void renderPage(String pdfPath, String outPath, int pageNumber, Rectangle area,
        ...
        ObjectExtractor oe = new ObjectExtractor(document, document.getPage(pageNumber + 1), true);
        
        Page page = oe.extractPage(pageNumber + 1);
        
        if (area != null) {
            page = page.getArea(area);
        }
        
        PDPage p = (PDPage) document.getDocumentCatalog().getPages().get(pageNumber);
        ...
        if (drawDetectedTables) {
            debugDetectedTables(g, document, page);
        }
